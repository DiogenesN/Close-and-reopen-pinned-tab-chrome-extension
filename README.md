...
# How it works
User has to manually specify a list of URLs that will be store in the browser storage and when the extension is activated (left click on the icon) it will close all the currently opened tabs (including the pinned tabs) except the currently active tab and it will open all the user-defined URLs as pinned tabs.
